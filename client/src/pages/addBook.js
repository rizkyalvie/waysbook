...
export default function Add() {
  ...
  const handleSubmit = useMutation(async (e) => {
    try {
      ...
      formData.set('bookattachment', form.attachment[0], form.attachment[0].name);
  ...
  return (
    <div className={styles.addForm}>
      ...
      <form onSubmit={(e) => handleSubmit.mutate(e)}>
        ...
        <input type="text" name="publicationdate" onChange={handleChange} placeholder="Publication Date" />
        <input type="text" name="author" onChange={handleChange} placeholder="Author" />
        <input type="text" name="pages" onChange={handleChange} placeholder="Pages" />
        ...
        <textarea name="description" onChange={handleChange} >About This Book</textarea>
        <label htmlFor="cover" className={styles.cover}>
          <p>Attach Book Cover</p>
          <img src={attach} alt="" />
          <input type="file" name="cover" onChange={handleChange} id="cover" hidden />
        </label>
        <label htmlFor="book" className={styles.cover}>
          <p>Attach Book File</p>
          <img src={attach} alt="" />
          <input type="file" name="attachment" onChange={handleChange} id="book" hidden />
        </label>
